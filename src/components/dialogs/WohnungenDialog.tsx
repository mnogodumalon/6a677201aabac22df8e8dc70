...
interface WohnungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Wohnungen['fields']) => Promise<void>;
  defaultValues?: Wohnungen['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function WohnungenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: WohnungenDialogProps) {
  const [fields, setFields] = useState<Partial<Wohnungen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'wohnungen');
      await onSubmit(clean as Wohnungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Wohnungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Wohnungen bearbeiten' : 'Wohnungen hinzufügen';
