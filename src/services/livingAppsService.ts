...
import type { Wohnungen, FritzBoxen, CreateWohnungen, CreateFritzBoxen } from '@/types/app';
...
export class LivingAppsService {
  // --- WOHNUNGEN ---
  static async getWohnungen(): Promise<Wohnungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WOHNUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Wohnungen[];
    return enrichLookupFields(records, 'wohnungen');
  }
  static async getWohnungenEntry(id: string): Promise<Wohnungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WOHNUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Wohnungen;
    return enrichLookupFields([record], 'wohnungen')[0];
  }
  static async createWohnungenEntry(fields: CreateWohnungen) {
    return callApi('POST', `/apps/${APP_IDS.WOHNUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'wohnungen') });
  }
  static async updateWohnungenEntry(id: string, fields: Partial<CreateWohnungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.WOHNUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wohnungen') });
  }
  static async deleteWohnungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WOHNUNGEN}/records/${id}`);
  }

  // --- FRITZ!BOXEN ---
  static async getFritzBoxen(): Promise<FritzBoxen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FRITZBOXEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as FritzBoxen[];
    return enrichLookupFields(records, 'fritz!boxen');
  }
  static async getFritzBoxenEntry(id: string): Promise<FritzBoxen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FRITZBOXEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as FritzBoxen;
    return enrichLookupFields([record], 'fritz!boxen')[0];
  }
  static async createFritzBoxenEntry(fields: CreateFritzBoxen) {
    return callApi('POST', `/apps/${APP_IDS.FRITZBOXEN}/records`, { fields: cleanFieldsForApi(fields as any, 'fritz!boxen') });
  }
  static async updateFritzBoxenEntry(id: string, fields: Partial<CreateFritzBoxen>) {
    return callApi('PATCH', `/apps/${APP_IDS.FRITZBOXEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'fritz!boxen') });
  }
  static async deleteFritzBoxenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FRITZBOXEN}/records/${id}`);
  }
