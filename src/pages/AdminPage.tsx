...
import { WohnungenViewDialog } from '@/components/dialogs/WohnungenViewDialog';
...
const WOHNUNGEN_FIELDS = [
  ...
  { key: 'mieter_vorname', label: 'Vorname', type: 'string/text' },
  { key: 'mieter_nachname', label: 'Nachname', type: 'string/text' },
...
const FRITZBOXEN_FIELDS = [
  { key: 'modell', label: 'Modell', type: 'string/text' },
  { key: 'seriennummer', label: 'Seriennummer', type: 'string/text' },
  { key: 'mac_adresse', label: 'MAC-Adresse', type: 'string/text' },
  { key: 'ip_adresse', label: 'IP-Adresse', type: 'string/text' },
  { key: 'firmware', label: 'Firmware-Version', type: 'string/text' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'inaktiv', label: 'Inaktiv' }, { key: 'defekt', label: 'Defekt' }, { key: 'in_wartung', label: 'In Wartung' }, { key: 'aktiv', label: 'Aktiv' }] },
  { key: 'installationsdatum', label: 'Installationsdatum', type: 'date/date' },
  { key: 'benutzername', label: 'Benutzername', type: 'string/text' },
  { key: 'admin_url', label: 'Administrations-URL', type: 'string/url' },
  { key: 'wohnung', label: 'Wohnung', type: 'applookup/select', targetEntity: 'wohnungen', targetAppId: 'WOHNUNGEN', displayField: 'bezeichnung' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'wohnungen', label: 'Wohnungen', pascal: 'Wohnungen' },
  { key: 'fritz!boxen', label: 'Fritz!Boxen', pascal: 'FritzBoxen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('wohnungen');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'wohnungen': return (data as any).wohnungen as Wohnungen[] ?? [];
      case 'fritz!boxen': return (data as any).fritzBoxen as FritzBoxen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'fritz!boxen':
        lists.wohnungenList = (data as any).wohnungen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'fritz!boxen' && fieldKey === 'wohnung') {
      const match = (lists.wohnungenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'wohnungen': return WOHNUNGEN_FIELDS;
      case 'fritz!boxen': return FRITZBOXEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'wohnungen': return {
        create: (fields: any) => LivingAppsService.createWohnungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateWohnungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteWohnungenEntry(id),
      };
      case 'fritz!boxen': return {
        create: (fields: any) => LivingAppsService.createFritzBoxenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFritzBoxenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFritzBoxenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'wohnungen' || dialogState?.entity === 'wohnungen') && (
        <WohnungenDialog
          open={createEntity === 'wohnungen' || dialogState?.entity === 'wohnungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'wohnungen' ? handleUpdate : (fields: any) => handleCreate('wohnungen', fields)}
          defaultValues={dialogState?.entity === 'wohnungen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Wohnungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Wohnungen']}
        />
      )}
      {(createEntity === 'fritz!boxen' || dialogState?.entity === 'fritz!boxen') && (
        <FritzBoxenDialog
          open={createEntity === 'fritz!boxen' || dialogState?.entity === 'fritz!boxen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'fritz!boxen' ? handleUpdate : (fields: any) => handleCreate('fritz!boxen', fields)}
          defaultValues={dialogState?.entity === 'fritz!boxen' ? dialogState.record?.fields : undefined}
          wohnungenList={(data as any).wohnungen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['FritzBoxen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['FritzBoxen']}
        />
      )}
      {viewState?.entity === 'wohnungen' && (
        <WohnungenViewDialog
          open={viewState?.entity === 'wohnungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'wohnungen', record: r }); }}
        />
      )}
      {viewState?.entity === 'fritz!boxen' && (
        <FritzBoxenViewDialog
          open={viewState?.entity === 'fritz!boxen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'fritz!boxen', record: r }); }}
          wohnungenList={(data as any).wohnungen ?? []}
        />
      )}
